...
import com.example.myapplication.util.ContextManager
import com.example.myapplication.data.export.ExportSession
import com.example.myapplication.data.export.toExportMessage
...
class ChatViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun sendMessage(content: String) {
        ...
        // 允许纯附件发送：只要有文字或有待发送附件，就允许发送
        if ((text.isBlank() && _pendingAttachments.value.isEmpty()) || currentSessionId == -1L || _isLoading.value) return
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                // 【修改4前置】先获取 pending 附件列表，供后续各分支使用
                val pending = attachmentRepo.getPendingBySessionId(currentSessionId)

                val provider = resolveProvider()
                if (provider == null) {
                    // 即使 provider 不可用，也要绑定附件到 userMessage 并清空输入区
                    pending.forEach { attachment ->
                        attachmentRepo.bindToMessage(attachment.id, userMessageId)
                        Log.d("ChatViewModel", "✓ 附件已绑定（provider不可用）: ${attachment.fileName} -> messageId=$userMessageId")
                    }
                ...
                if (!provider.enabled) {
                    // 即使 provider 被禁用，也要绑定附件到 userMessage 并清空输入区
                    pending.forEach { attachment ->
                        attachmentRepo.bindToMessage(attachment.id, userMessageId)
                        Log.d("ChatViewModel", "✓ 附件已绑定（provider被禁用）: ${attachment.fileName} -> messageId=$userMessageId")
                    }
                    _pendingAttachments.value = emptyList()
                ...
                // 【修改4】附件绑定提前执行：在获取 pending 后立即绑定到 userMessage，不再等待 onComplete
                pending.forEach { attachment ->
                    attachmentRepo.bindToMessage(attachment.id, userMessageId)
                    Log.d("ChatViewModel", "✓ 附件已在发送前绑定: ${attachment.fileName} -> messageId=$userMessageId")
                }
                _pendingAttachments.value = emptyList()
                // 【修改5】使用 ContextManager 统一构建上下文
                // 1. 提取世界书匹配逻辑
                ...
                // 2. 调用 ContextManager 构建上下文
                val contextResult = ContextManager.buildContext(
                    preset = preset,
                    character = _currentCharacter.value,
                    worldBookEntries = matchedEntries,
                    attachments = pending,
                    historyMessages = if (sendHistory) localMessages.takeLast(maxHistoryCount) else emptyList(),
                    userInput = text,
                    contextLimit = null // 使用默认限制
                )

                val historyMessages = contextResult.messages
                ...
                chatProvider.streamChat(
                    ...
                    onComplete = {
                        ...
                        // 【修改5】onComplete 移除重复绑定：附件已在发送前立即绑定，此处仅记录日志
                        Log.d("ChatViewModel", "=== 发送消息完成 === (附件已在发送前立即绑定)")
                    ...
                    onError = { error ->
                        ...
                        // 【修改6】onError 添加日志说明：附件已在发送前绑定，确保状态一致性有据可查
                        Log.d("ChatViewModel", "✓ 请求出错但附件已绑定: $error (pending count=${pending.size})")
         ...
         viewModelScope.launch {
             try {
                 ...
                 if (extractedText == null || extractedText.isBlank()) {
                     Log.e("ChatViewModel", "文件读取失败或为空：$fileName")
                     _errorMessages.emit("文件读取失败：$fileName")
                     return@launch
                 }
                 
                 // 复制文件到私有缓存目录
                 val cachedFile = copyFileToCache(uri, fileName)
                 ...
                 val attachmentId = attachmentRepo.insert(attachment)
                 Log.d("ChatViewModel", "附件已添加：$fileName (id=$attachmentId, size=${extractedText.length})")
                 _pendingAttachments.value = attachmentRepo.getPendingBySessionId(currentSessionId)
             } catch (e: Exception) {
                 Log.e("ChatViewModel", "添加附件异常", e)
    ...
    private fun copyFileToCache(uri: Uri, fileName: String): java.io.File {
        val cacheDir = java.io.File(getApplication<ChatApp>().cacheDir, "attachments")
        if (!cacheDir.exists()) cacheDir.mkdirs()
        
        val timestamp = System.currentTimeMillis()
        val cachedFile = java.io.File(cacheDir, "${timestamp}_$fileName")
        
        getApplication<ChatApp>().contentResolver.openInputStream(uri)?.use { input ->
    ...
    private suspend fun handleSendError(message: String) {
        ...
        // 【修改7】handleSendError 清空附件：覆盖 catch 异常路径，确保所有错误出口都不会让附件残留
        _pendingAttachments.value = emptyList()
    ...
    fun regenerateMessage(messageId: Long) {
     ...
     suspend fun getAttachmentsByMessageId(messageId: Long): List<AttachmentEntity> {
         return attachmentRepo.getBySessionId(currentSessionId).filter { it.messageId == messageId }
     }

     /** 导出当前会话 */
     suspend fun exportSession(includeAttachments: Boolean): ExportSession? {
         if (currentSessionId == -1L) return null
         
         val session = sessionRepo.getSessionById(currentSessionId) ?: return null
         val messages = messageRepo.getMessagesOnce(currentSessionId)
         val allAttachments = if (includeAttachments) {
             attachmentRepo.getBySessionId(currentSessionId)
         } else {
             emptyList()
         }
         
         val exportMessages = messages.map { message ->
             val messageAttachments = if (includeAttachments) {
                 allAttachments.filter { it.messageId == message.id }
             } else {
                 emptyList()
             }
             message.toExportMessage(messageAttachments)
         }
